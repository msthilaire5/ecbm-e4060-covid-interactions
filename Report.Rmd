...
For 50 simulations, we find 20 genes. 
With only 50 simulations, we can observe some differences between the paper and our results. 
PABPC3 gene appears in the paper but not in our results and NOVA2 and ENOX1 genes appear in our results but not in the paper. However, the genes with very small p-value are in both tables. 
...
barplot(genome_hits$pval, main = "Pvalue for genome hits",las=2,horiz=F, col = colors, cex.main = 1, cex.names = 0.7, ylab="number of sites", xlab="Gene_name", names.arg=genome_hits$Gene_name)

barplot(genome_hits$N, main = "Number of sites for genome hits",las=2,horiz=F, col = colors, cex.main = 1, cex.names = 0.7, ylab="number of sites", xlab="Gene_name", names.arg=genome_hits$Gene_name)
